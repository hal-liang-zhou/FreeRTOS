...
#define tskSTATIC_AND_DYNAMIC_ALLOCATION_POSSIBLE \
  (((configSUPPORT_STATIC_ALLOCATION == 1) && (configSUPPORT_DYNAMIC_ALLOCATION == 1)) || (portUSING_MPU_WRAPPERS == 1))
...
{
  volatile StackType_t *pxTopOfStack; /*< Points to the location of the last item placed on the tasks stack.  THIS MUST
                                         BE THE FIRST MEMBER OF THE TCB STRUCT. */
  ...
  ListItem_t xStateListItem; /*< The list that the state list item of a task is reference from denotes the state of that
                                task (Ready, Blocked, Suspended ). */
  ListItem_t xEventListItem; /*< Used to reference a task from an event list. */
  UBaseType_t uxPriority;    /*< The priority of the task.  0 is the lowest priority. */
  StackType_t *pxStack;      /*< Points to the start of the stack. */
  char pcTaskName[configMAX_TASK_NAME_LEN];
      /*< Descriptive name given to the task when created.  Facilitates debugging only. */ /*lint !e971 Unqualified char
...
tskSTATIC_AND_DYNAMIC_ALLOCATION_POSSIBLE. */
  ...
  uint8_t ucStaticallyAllocated; /*< Set to pdTRUE if the task is a statically allocated to ensure no attempt is made to
                                    free the memory. */
#endif
...
PRIVILEGED_DATA static volatile TickType_t
...
PRIVILEGED_DATA static uint32_t
                                                                    ...
                                                                    strings and single characters only. */
{
  ...
  if ((pxTaskBuffer != NULL) && (puxStackBuffer != NULL))
  {
    ...
    function - use them. */
    pxNewTCB = (TCB_t *)pxTaskBuffer; /*lint !e740 Unusual cast is ok as the structures are designed to have the same
    ...
    pxTopOfStack = (StackType_t *)(((portPOINTER_SIZE_TYPE)pxTopOfStack) &
                                   (~((portPOINTER_SIZE_TYPE)
                                          portBYTE_ALIGNMENT_MASK))); /*lint !e923 MISRA exception.  Avoiding casts
                                                                         between pointers and integers is not practical.
...
/* Add the idle task at the lowest priority. */
#if (configSUPPORT_STATIC_ALLOCATION == 1)
  {
    ...
    xIdleTaskHandle = xTaskCreateStatic(prvIdleTask, "IDLE", ulIdleTaskStackSize, (void *)NULL,
...
#else
  {
    ...
    xReturn = xTaskCreate(prvIdleTask, "IDLE", configMINIMAL_STACK_SIZE, (void *)NULL,
                          ...
                          &xIdleTaskHandle); /*lint !e961 MISRA exception, justified as it is not a redundant explicit
  ...
  vTaskSuspendAll();
  {
    ...
    do
    {
      ...
      if (pxTCB != NULL)
      {
        ...
        uxQueue >
        (UBaseType_t)tskIDLE_PRIORITY); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
    ...
    if (xConstTickCount >= xNextTaskUnblockTime)
    {
      for (;;)
      {
        if (listLIST_IS_EMPTY(pxDelayedTaskList) != pdFALSE)
        {
          ...
          next time through. */
          xNextTaskUnblockTime = portMAX_DELAY; /*lint !e961 MISRA exception as the casts are only redundant for some
                                                   ports. */
          break;
        }
        else
  ...
  taskENTER_CRITICAL();
  {
                                                                ...
                                                                within pre-processor directives clearer. */
    {
      ...
      xReturn = pdTRUE;
             ...
             *pxTicksToWait) /*lint !e961 Explicit casting is only redundant with some compilers, whereas others require
                                it to prevent integer conversion errors. */
    {
...
to the task to free any memory allocated at the application level. */
...
#elif (tskSTATIC_AND_DYNAMIC_ALLOCATION_POSSIBLE == 1)
  {
    ...
    else
    {
      ...
      configASSERT(pxTCB->ucStaticallyAllocated == tskSTATICALLY_ALLOCATED_STACK_AND_TCB) mtCOVERAGE_TEST_MARKER();
    }
  }
#endif /* configSUPPORT_DYNAMIC_ALLOCATION */
}
  ...
  if (pxTaskStatusArray != NULL)
  {
    ...
    for (x = 0; x < uxArraySize; x++)
    {
      ...
      sprintf(pcWriteBuffer, "\t%c\t%u\t%u\t%u\r\n", cStatus, (unsigned int)pxTaskStatusArray[x].uxCurrentPriority,
              (unsigned int)pxTaskStatusArray[x].usStackHighWaterMark, (unsigned int)pxTaskStatusArray[x].xTaskNumber);
